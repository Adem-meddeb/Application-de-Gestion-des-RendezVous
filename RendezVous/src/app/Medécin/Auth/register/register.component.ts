...
export class RegisterComponent implements OnInit, AfterViewInit {
  ...
  paymentMethodsBackend = ['CASH', 'CREDIT_CARD', 'CHEQUE', 'INSURANCE', 'BANK_TRANSFER'];
  ...
  constructor(
    ...
    this.registrationForm = this.fb.group({
      ...
      profilePhoto: this.fb.group({
        file: [null, [
          Validators.required,
          this.fileTypeValidator(['image/jpeg', 'image/jpg', 'image/png', 'application/pdf']),
          this.fileSizeValidator(5120) // 5MB
        ]],
        fileName: [''],
        fileSize: [0],
        preview: ['']
  ...
  private prepareFormData(): FormData {
    ...
    // Récupération des fichiers avec sécurité
    const profilePhotoFile = this.profilePhoto.get('file')?.value ?? null;
    const aptitudeCertFile = (this.experience.get('certification') as FormGroup)?.get('file')?.value ?? null;
    ...
    if (aptitudeCertFile) {
      formData.append('aptitudeCertificate', aptitudeCertFile);
    }
    ...
    const doctorRequest = {
      ...
      cin: formValue.personalInfo.cin,
      gender: formValue.personalInfo.gender.toUpperCase(),
      age: formValue.personalInfo.age,
      phoneNumber: formValue.personalInfo.phone,
      email: formValue.personalInfo.email,
      billingAddress: formValue.personalInfo.address,
      password: formValue.personalInfo.password,
      office: {
        address: formValue.cabinet.address,
        governorate: formValue.cabinet.governorate,
        longitude: formValue.cabinet.longitude,
        latitude: formValue.cabinet.latitude
      },
      specializationIds: [1, 2], // À adapter
      languages: formValue.schedule.languages,
      paymentMethods: formValue.schedule.paymentMethods.map((m: string) => this.translatePaymentMethod(m)),
      education: formValue.education.degrees.map((d: any) => ({
        name: d.degreeName,
        establishment: d.institution,
        year: d.year
      })),
      experience: formValue.experience.positions.map((p: any) => ({
        name: p.position,
        establishment: p.hospital,
        duration: p.duration + ' ans'
      }))
    };

    // Ajout du DTO au FormData
    const doctorRequestBlob = new Blob([JSON.stringify(doctorRequest)], { type: 'application/json' });
    formData.append('doctorRequest', doctorRequestBlob);
  ...
  translatePaymentMethodForDisplay(method: string): string {
    const translations: { [key: string]: string } = {
      'CASH': 'Espèces',
